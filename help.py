...
def get_surfaces(pts):
    ...
    return sorted(
        arr,
        key=lambda x: np.sqrt(np.sum((rectangle_center([p[1] for p in x])) ** 2)),
        reverse=True,
    )
...
#ground functions:
def generate_ground(grid_size=20, step_size=3):
    ground = []
    for i in range(-grid_size // 2, grid_size // 2):
        for j in range(-grid_size // 2, grid_size // 2):
            points = np.array(
                [
                    [i * step_size, 3, j * step_size],
                    [(i + 1) * step_size, 3, j * step_size],
                    [(i + 1) * step_size, 3, (j + 1) * step_size],
                    [i * step_size, 3, (j + 1) * step_size],
                ]
            )
